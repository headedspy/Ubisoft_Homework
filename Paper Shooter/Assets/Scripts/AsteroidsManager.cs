...
public class AsteroidsManager : MonoBehaviour {
    ...
    void Update ()
    {
		...
		if (Random.Range (0, 651) == 5 && !hasBonus) {
			Instantiate (bonus [Random.Range (0, 3)], new Vector3 (Random.Range (-7f, 7f), -0.32f, Random.Range (-12f, 15f)), Quaternion.identity);
			hasBonus = true;
		}
	}
    void OnDrawGizmos()
    ...
    private void SpawnNewAsteroids()
    {
            ...
            Time.time > nextAsteroidSpawnTime)
        {
			...
			newAsteroid.GetComponent<MeshFilter>().sharedMesh = asteroidMesh[Random.Range(0,2)];                     //NEW
